 ...
 */
@WebServlet(urlPatterns="/town/search_frequency")
public class SearchServlet_frequency extends HttpServlet {
	private static final long serialVersionUID = 1L;


	/**
	 *　検索機能。サークル一覧を取得し、一覧結果へフォワードする(頻度を重視する)
	 */
	protected void doGet(HttpServletRequest request, HttpServletResponse response)
			throws ServletException, IOException {

		System.out.println("SearchServlet_frequency.java_DoGet()");
		//DAOの取得
		try(UserDAO dao = new UserDAO()) {
			//サークルのリストを一覧で取得し、リクエスト属性へ格納する
			List<Circle> list = dao.circleList_frequency();
